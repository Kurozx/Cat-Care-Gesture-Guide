...
import { Button, Card, Empty, Icon, colors, s } from './src/ui';
...
function Application() {
  ...
  const navigate = (next: Route) => { app.setNotice(''); setStack(s => [...s, next]); };
  const back = () => setStack(s => s.length > 1 ? s.slice(0, -1) : [{ name: 'guides' }]);
  ...
  return <SafeAreaView style={{ flex: 1, backgroundColor: colors.cream }}><StatusBar style="dark" /><View style={{ width: '100%', maxWidth: 600, alignSelf: 'center', flex: 1 }}>
    <View style={{paddingHorizontal:22,paddingVertical:14,flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}><Pressable accessibilityRole="button" accessibilityLabel="หน้าแรก" onPress={()=>navigate({name:'welcome'})} style={s.row}><View style={{backgroundColor:colors.orange,borderRadius:12,padding:8}}><Icon name="cat" color="#fff" size={23}/></View><Text style={{color:colors.orange,fontSize:22,fontWeight:'900',letterSpacing:-0.8}}>CATCARE</Text></Pressable><View style={{flexDirection:'row',gap:8}}>{route.name!=='welcome'&&<Pressable accessibilityRole="button" accessibilityLabel="ย้อนกลับ" onPress={back} style={s.headerIcon}><Icon name="arrow-left" size={21} color={colors.dark}/></Pressable>}<Pressable accessibilityRole="button" accessibilityLabel="รีเฟรชข้อมูล" onPress={()=>void app.refresh()} style={s.headerIcon}><Icon name="refresh" size={21}/></Pressable><Pressable accessibilityRole="button" accessibilityLabel="เปิดบัญชีของฉัน" onPress={()=>navigate({name:app.mode==='guest'?'auth':'account'})} style={[s.headerIcon,{backgroundColor:colors.pale}]}><Icon name="account-circle-outline" size={26}/></Pressable></View></View>
