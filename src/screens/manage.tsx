...
export function CatForm({ cat, back }: ScreenProps & { cat?: Cat }) {
  ...
  const [home, setHome] = useState(cat?.home_id ?? homes[0]?.id ?? ''); const [name, setName] = useState(cat?.name ?? ''); const [breed, setBreed] = useState(cat?.breed ?? ''); const [birth, setBirth] = useState(cat?.birth_date ?? ''); const [chip, setChip] = useState(cat?.microchip_code ?? ''); const [notes, setNotes] = useState(cat?.notes ?? ''); const [confirm, setConfirm] = useState(false);
  return <View style={{ gap: 20 }}><Heading title={cat ? `โปรไฟล์ ${cat.name}` : 'เพิ่มเพื่อนตัวน้อย'} /><Text style={s.label}>บ้านของแมว</Text><View style={s.wrap}>{homes.filter(h => !cat || h.id === cat.home_id).map(h => <Chip key={h.id} text={h.name} active={home === h.id} onPress={() => setHome(h.id)} />)}</View><Field label="ชื่อแมว" value={name} onChangeText={setName} maxLength={80} /><Field label="สายพันธุ์" value={breed} onChangeText={setBreed} /><Field label="วันเกิด (YYYY-MM-DD ปี ค.ศ. ไม่บังคับ)" value={birth} onChangeText={setBirth} placeholder="2023-03-10" /><Field label="หมายเลขไมโครชิป (ไม่บังคับ)" value={chip} onChangeText={setChip} /><Field label="ข้อมูลเพิ่มเติม" value={notes} onChangeText={setNotes} multiline />{!!task.error && <Text style={s.error}>{task.error}</Text>}<Button title="บันทึกโปรไฟล์แมว" loading={task.busy} onPress={() => void task.run(async () => { if (!home) throw new Error('กรุณาสร้างบ้านก่อน'); const birthDate = dateOnly(birth); if (birthDate && birthDate > localInput().slice(0, 10)) throw new Error('วันเกิดต้องไม่อยู่ในอนาคต'); await repo.saveCat(app.mode, { home_id: home, name: name.trim(), breed: breed.trim(), birth_date: birthDate, microchip_code: chip.trim(), notes: notes.trim() }, cat?.id); await app.refresh(); app.setNotice('บันทึกโปรไฟล์แมวแล้ว'); back(); })} />
